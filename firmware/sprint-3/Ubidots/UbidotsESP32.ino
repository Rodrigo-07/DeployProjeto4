 ...
 * Definir Constantes
 ****************************************/
const char *UBIDOTS_TOKEN = "BBUS-qrZs2fn8JoMWkCNcoKdA0h23EH4szb";  // Insira aqui o seu TOKEN do Ubidots
const char *WIFI_SSID = "Iphone";      // Insira aqui o SSID da sua rede Wi-Fi
const char *WIFI_PASS = "rodrigo123";      // Insira aqui a senha da sua rede Wi-Fi
const char *DEVICE_LABEL = "ESP32";   // Insira aqui a etiqueta do seu dispositivo para a qual os dados serão publicados
const char *LATITUDE = "Latitude"; // Variável que define a latitude localizada
...
void setup() // Função que inicializa o ESP32
{
  ...
  ubidots.reconectar(); // Conecta o UBIDOTS
...
while (Serial2.available() > 0) // Enquanto a leitura do serial2 for menor que 0...
  ...
  if (millis() > 5000 && gps.charsProcessed() < 10) // Se o tempo for maior que 5 segundos e os caracteres processados forem menos que 10
  {
    ...
    while (true); // Torna o While True
  }
  // Coloque seu código principal aqui, para ser executado repetidamente:
  if (!ubidots.conectado()) // Confere se o ubidots está conectado
  {
    ubidots.reconectar(); // reconecta com o ubidots
  }

  delay(5000); // aguarda 5 segundos

  ubidots.adicionar(LATITUDE, lat); // Insere as etiquetas das variáveis e o valor a ser enviado
  ubidots.adicionar(LONGITUDE, longi); // Insere as etiquetas das variáveis e o valor a ser enviado
  ubidots.publicar(DEVICE_LABEL); // Publica a variável indicada
